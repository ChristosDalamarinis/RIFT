...
WINDOW_WIDTH = 1200
WINDOW_HEIGHT = 900
BACKGROUND_COLOR = [0.0, 0.0, 0.0]  # Negative values -> darkness
...
THREE_OCLOCK_FLICKER_FREQUENCY = 120  # Hz
...
for hour in clock_positions:
    ...
    # Determine texture type: square wave for 3 o'clock, sine wave for others
    if hour == 3:
        texture_type = 'square'  # Square wave grating
    else:
        texture_type = 'sin'  # Sinusoidal grating 
    
    # Create Gabor patch
    gabors[hour] = visual.GratingStim(
        win,
        tex=texture_type,  # Use determined texture type
...
# This script is an iteration of g1.py where the grey pixels of the gabor patches have been eliminated