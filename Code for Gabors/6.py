...
GABOR_SF = 0.05  # Spatial frequency - SMALLER: wider stripes; BIGGER: narrower stripes - 0.05 by default
...
print("\n" + "="*70)
print("8 GABOR PATCHES - CHROMATIC OPPONENT FLICKER (g6.py)")
print("="*70)
...
while not trial_ended:
    ...
    for hour in clock_positions:
        if hour == 9 and ENABLE_NINE_OCLOCK_FLICKER:
            # Flicker between GREEN and MAGENTA (processed colors)
            current_color = calculate_color_flicker_frame(
                frame_num,
                NINE_OCLOCK_FLICKER_FREQUENCY,
                REFRESH_RATE,
                COLOR_A_FINAL,  # Processed green
                COLOR_B_FINAL   # Processed magenta
            )
...
# compared to 5.py this script adds only saturation of colours